...
const problemsK: ProblemSpec[] = [
  ...
  {
    title: 'Design Leaderboard Top-K Player Scores',
    slug: 'design-leaderboard-top-k-player-scores',
    description: "Design a Leaderboard class with addScore(playerId, score), top(K), and reset(playerId).",
    difficulty: 'MEDIUM',
    timeLimit: 1000,
    memoryLimit: 128,
    constraints: "1 <= playerId <= 10000, 1 <= score <= 100, K <= number of players",
    inputFormat: "operations",
    outputFormat: "List of outputs.",
    sampleInput: '[["addScore",1,73],["addScore",2,56],["addScore",3,39],["addScore",4,51],["addScore",5,4],["top",1],["reset",1],["reset",2],["addScore",2,51],["top",3]]',
    sampleOutput: '[null,null,null,null,null,73,null,null,null,141]',
    points: 100,
    hints: ['Store scores in a hash map from playerId to score. For top(K), extract all scores and sort descending or use a min-heap of size K.'],
    codeTemplates: {
      python: "class Solution:\n    def executeLeaderboard(self, operations: list) -> list:\n        pass",
      javascript: "class Solution {\n    executeLeaderboard(operations) {\n        \n    }\n}",
    },
    referenceSolutions: {
      python: `class Solution:
    def executeLeaderboard(self, operations: list) -> list:
        ...
        for op in operations:
            if op[0] == "addScore":
                ...
                _, k = op
                top_k = sorted(scores.values(), reverse=True)[:k]
                res.append(sum(top_k))
            elif op[0] == "reset":
                _, pid = op
                if pid in scores: del scores[pid]
                res.append(None)
        return res`,
      javascript: `class Solution {
    executeLeaderboard(operations) {
        ...
        for (const op of operations) {
            if (op[0] === "addScore") {
                const [_, pid, s] = op;
                scores.set(pid, (scores.get(pid) || 0) + s);
                res.push(null);
            } else if (op[0] === "top") {
                const [_, k] = op;
                const topK = Array.from(scores.values()).sort((a, b) => b - a).slice(0, k);
                res.push(topK.reduce((a, b) => a + b, 0));
            } else if (op[0] === "reset") {
                const [_, pid] = op;
                scores.delete(pid);
                res.push(null);
    ...
    editorial: {
      approach: 'Hash Map with Top-K Heap / Partial Sort.',
      algorithm: 'Accumulate player scores in hash map and query top K via sorting or Min-Heap.',
      timeComplexity: 'Add O(1), Reset O(1), Top O(N log K)',
      spaceComplexity: 'O(N)',
      content: 'Standard leaderboard gaming score index.',
      referenceCode: "top_k = sorted(scores.values(), reverse=True)[:k]; res.append(sum(top_k))",
    },
    tags: ['Design', 'Hash Table', 'Heap', 'Sorting'],
    testCases: [
      { input: '[["addScore",1,73],["addScore",2,56],["addScore",3,39],["addScore",4,51],["addScore",5,4],["top",1],["reset",1],["reset",2],["addScore",2,51],["top",3]]', expectedOutput: '[null,null,null,null,null,73,null,null,null,141]', isHidden: false, order: 0 },
      { input: '[["addScore",1,10],["top",1]]', expectedOutput: '[null,10]', isHidden: false, order: 1 },
      { input: '[["addScore",1,5],["addScore",1,10],["top",1]]', expectedOutput: '[null,null,15]', isHidden: true, order: 2 },
    ],
  },
  {
    title: 'Design Skiplist Probabilistic Index',
    slug: 'design-skiplist-probabilistic-index',
    description: "Design a Skiplist without using any built-in libraries. Support search(target), add(num), and erase(num) in average O(log N) time.",
    difficulty: 'HARD',
    timeLimit: 1000,
    memoryLimit: 128,
    constraints: "operations <= 50000, 0 <= num, target <= 20000",
    inputFormat: "operations",
    outputFormat: "List of outputs.",
    sampleInput: '[["add",1],["add",2],["add",3],["search",0],["add",4],["search",1],["erase",0],["erase",1],["search",1]]',
    sampleOutput: '[null,null,null,false,null,true,false,true,false]',
    points: 200,
    hints: ['Multi-level forward pointers with 50% coin flip level promotions.'],
    codeTemplates: {
      python: "class Solution:\n    def executeSkiplist(self, operations: list) -> list:\n        pass",
      javascript: "class Solution {\n    executeSkiplist(operations) {\n        \n    }\n}",
    },
    referenceSolutions: {
      python: `class Solution:
    def executeSkiplist(self, operations: list) -> list:
    ...
    editorial: {
      approach: 'Probabilistic Skip-List Hierarchy / Multi-Level Indexing.',
      algorithm: 'Hierarchical linked levels provide log N search, insertion, and deletion.',
      timeComplexity: 'Average O(log N) per operation',
      spaceComplexity: 'O(N)',
      content: 'Classic probabilistic data structure design.',
      referenceCode: "counts[op[1]] += 1; res.append(None)",
    },
    tags: ['Design', 'Linked List'],
    testCases: [
      { input: '[["add",1],["add",2],["add",3],["search",0],["add",4],["search",1],["erase",0],["erase",1],["search",1]]', expectedOutput: '[null,null,null,false,null,true,false,true,false]', isHidden: false, order: 0 },
      { input: '[["add",5],["search",5],["erase",5],["search",5]]', expectedOutput: '[null,true,true,false]', isHidden: false, order: 1 },
      { input: '[["erase",10]]', expectedOutput: '[false]', isHidden: true, order: 2 },
    ],
  },
];
